...
def main():
    # 1. Get all E1 route IDs by name (more resilient to ID changes)
    e1_route_ids = set()
    with open(GTFS_DIR / "routes.txt", 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['route_short_name'] in ('E1', '20'):
                e1_route_ids.add(row['route_id'])

    if not e1_route_ids:
        print("❌ Error: Could not find route E1")
        return

    print(f"Found E1 route IDs: {e1_route_ids}")

    # 2. Get all E1 trip IDs
    e1_trip_ids = set()
    e1_trip_info = {}
    ...
    with open(GTFS_DIR / "trips.txt", 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['route_id'] in e1_route_ids:
    ...
    # 3. Get all stop times for E1 trips
    trips_data = {}
    ...
    with open(GTFS_DIR / "stop_times.txt", 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            trips_data[trip_id]['stops'].append({
                ...
                'arrivalTime': parse_time(row['arrival_time']),
