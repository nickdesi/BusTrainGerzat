...
## 2026-05-29 - O(N) Anti-Pattern: Propagating Arrays for Lookups in React Components
**Learning:** Found an anti-pattern where an array (`favorites`) was mapped into a new array (`favorites.map()`), which was then converted into a `Set` via `useMemo` in multiple child components (`DeparturesBoard`, `DeparturesList`) and hooks (`useDelayNotifications`). This caused repeated O(N) array mapping allocations and redundant `new Set()` instantiations.
**Action:** Consolidate `Set` creation in the parent component using a single `for...of` loop (to avoid the intermediate mapped array) and pass the resulting pre-computed `Set` object down the component tree as a prop.
**Action:** When passing complex objects like Sets to memoized components as default props, always use a stable, file-scoped constant (`const EMPTY_SET = new Set()`) instead of inline defaults (`favorites = new Set()`) to prevent breaking `React.memo` and `useMemo` dependency arrays.
