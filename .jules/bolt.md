...
## 2026-05-24 - Avoiding multiple array passes for data transformations
**Learning:** Found an anti-pattern in `src/app/api/trip/[tripId]/route.ts` where multiple passes were made over the same data. First, `staticTrip.stops.map(...)` was used to transform the array, creating an intermediate array. Then, `getCurrentStopIndex` performed an internal `.findIndex()`, scanning the new array again. Finally, a `for (const [index, stop] of stops.entries())` loop iterated over the array a third time to update statuses. This leads to redundant memory allocations and excessive iteration overhead, making the time complexity `O(3N)` effectively and adding GC pressure on a hot API path.
**Action:** Replace sequential `.map()`, `.findIndex()`, and `.entries()` calls on the same array sequence with a single, unified `for` loop that performs mapping, tracks the needed status/index logic incrementally, and populates the final array directly. This reduces the operation to strict `O(N)` with no intermediate arrays.
