...
**Learning:** Found an anti-pattern in `rateLimit` cache cleanup where `[...tokenCache.entries()].sort(...)` was used. Using the spread operator on an iterator like `.entries()` creates an enormous array of array tuples in memory. Sorting this large intermediate array and then slicing it (`.slice()`) creates further unnecessary memory allocations and garbage collection pressure, particularly during hourly cleanup spikes. Also learned that while Map preserves insertion order, it cannot be used safely for oldest-first cache eviction if keys are updated in place, because updating an existing key does not change its insertion position.
**Action:** Replace `[...map.entries()]` spread operations and chained `.slice()` calls with a single-pass `for...of` loop to extract only the necessary fields into a typed object array, which is then sorted and iterated directly for deletion. Avoid relying on Map insertion order for expiration logic if keys can be updated.
